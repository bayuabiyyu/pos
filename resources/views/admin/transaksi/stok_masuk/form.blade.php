...
@section('content')
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
       STOK
        <small>Transaksi Stok Masuk</small>
      </h1>
      <ol class="breadcrumb">
    ...
    <!-- form start -->
    <form action="{{ $data['stok_masuk']->exists ? route('stokmasuk.update', $data['stok_masuk']->id) : route('stokmasuk.store') }}" role="form" id="form_stokmasuk" method="{{ $data['stok_masuk']->exists ? 'PUT' : 'POST' }}">
        <div class="row">
        <!-- left column -->
        <div class="col-md-12">
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Transaksi Stok Masuk</h3>
            </div>
            <!-- /.box-header -->
            <div class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-3">
                            <input type="text" class="form-control" id="kode_barang" name="kode_barang" placeholder="Kode Barang" value="{{ $data['stok_masuk']->exists ? $data['stok_masuk']->kode_barang : '' }}" readonly>
                        </div>
                        <div class="col-sm-3">
                            <input type="text" class="form-control" id="nama_barang" name="nama_barang" placeholder="Nama Barang" value="{{ $data['stok_masuk']->exists ? $data['stok_masuk']->nama_barang : '' }}" readonly>
                        </div>
                        @if (!$data['stok_masuk']->exists)
                            <div class="col-sm-1">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-2">
                            <input type="number" class="form-control" id="qty" name="qty" placeholder="Qty" value="{{ $data['stok_masuk']->exists ? $data['stok_masuk']->qty : '' }}">
                        </div>
                    </div>
                        ...
                        <div class="col-sm-5">
                            <select class="form-control" id="kode_supplier" name="kode_supplier">
                               @foreach ($data['supplier'] as $item)
                                <option value="{{ $item->kode_supplier }}" {{ $data['stok_masuk']->exists && $data['stok_masuk']->kode_supplier == $item->kode_supplier ? 'selected' : '' }} >{{ $item->nama_supplier }}</option>
                               @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-4">
                            <textarea class="form-control" name="keterangan" id="keterangan" cols="" rows="">{{ $data['stok_masuk']->exists ? $data['stok_masuk']->keterangan : '' }}</textarea>
                        </div>
                    </div>
...
$(document).ready(function(){
    ...
    // SUBMIT FORM //
    $('#form_stokmasuk').on('submit', function(e){
        e.preventDefault();
        var me          = $(this),
            url         = me.attr('action'),
            ...
            $.ajax({
                ...
                success: function(res){
                    ...
                    if(res.success == true){
                        me.trigger('reset');
                        if(method == "PUT"){
                            window.location.href = "{{ route('stokmasuk.index') }}";
                        }
                    }
                },
  ...
  // TAMPILKAN LIST DATABARANG //
    $('#btn_search').on('click', function(e){
        e.preventDefault();
        var me          = $(this),
            url         = "{!! route('stokmasuk.data_barang'); !!}",
            method      = "POST",
            dataType    = "HTML";
